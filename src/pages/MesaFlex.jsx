...
import { 
  Cable, Columns, Store, ShoppingCart, 
...
export default function MesaFlex() {
  ...
  const [visibilidadeColunas, setVisibilidadeColunas] = useState(() => {
    const saved = localStorage.getItem('visibilidade_colunas_flex_v4');
  ...
  const [fornecedoresGlobais, setFornecedoresGlobais] = useState(() => {
    const saved = localStorage.getItem('fornecedoresGlobais_flex_v4');
    return saved ? JSON.parse(saved) : [];
  });

  const [colunasCodVisiveis, setColunasCodVisiveis] = useState(() => {
    const saved = localStorage.getItem('colunas_cod_visiveis_flex_v4');
    return saved ? JSON.parse(saved) : {};
  });

  const [coresFornecedores, setCoresFornecedores] = useState(() => {
    const saved = localStorage.getItem('cores_fornecedores_flex_v4');
  ...
  const [lojasGlobais, setLojasGlobais] = useState(() => {
    const saved = localStorage.getItem('lojasGlobais_flex');
  ...
  useEffect(() => { localStorage.setItem('visibilidade_colunas_flex_v4', JSON.stringify(visibilidadeColunas)); }, [visibilidadeColunas]);
  useEffect(() => { localStorage.setItem('fornecedoresGlobais_flex_v4', JSON.stringify(fornecedoresGlobais)); }, [fornecedoresGlobais]);
  useEffect(() => { localStorage.setItem('colunas_cod_visiveis_flex_v4', JSON.stringify(colunasCodVisiveis)); }, [colunasCodVisiveis]);
  useEffect(() => { localStorage.setItem('cores_fornecedores_flex_v4', JSON.stringify(coresFornecedores)); }, [coresFornecedores]);
  useEffect(() => { localStorage.setItem('lojasGlobais_flex', JSON.stringify(lojasGlobais)); }, [lojasGlobais]);
  ...
  const carregarMesa = async () => {
    ...
    try {
      const t = Date.now();
      let url = `/mesacompras/FLEX?page=${paginaAtual}&size=${ITENS_POR_PAGINA}&_t=${t}`;
  ...
  const carregarMarcasGlobais = async () => {
    try {
      const res = await api.get(`/mesacompras/marcas/FLEX?_t=${Date.now()}`);
  ...
  const handleSalvarNovoProduto = async () => {
    ...
    const payload = {
      ...
      categoria: 'FLEX', categoriaAba: 'FLEX',
  ...
  return (
    <div className="h-full flex flex-col bg-[#0B1120] p-6 overflow-hidden text-slate-200 relative">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6 shrink-0">
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-wider flex items-center gap-3">
            <Cable className="text-emerald-400" size={32} /> MESA DE COMPRAS: FLEX
          </h1>
      ...
      {/* 🔥 MODAL DE VISIBILIDADE DAS COLUNAS FIXAS */}
            ...
            <div className="flex items-start justify-between mb-4 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-bold mt-0.5">Ligue ou desligue as colunas fixas da sua mesa.</p>
                </div>
              </div>
              <button onClick={() => setModalAtivo(null)} className="text-slate-500 hover:text-white p-2 rounded-xl bg-slate-800/50 transition-colors"><X size={20} /></button>
            </div>
