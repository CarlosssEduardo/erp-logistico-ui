...
import { 
  Battery, Columns, Store, ShoppingCart, 
...
export default function MesaBaterias() {
  ...
  const [visibilidadeColunas, setVisibilidadeColunas] = useState(() => {
    const saved = localStorage.getItem('visibilidade_colunas_baterias_v4');
  ...
  const [fornecedoresGlobais, setFornecedoresGlobais] = useState(() => {
    const saved = localStorage.getItem('fornecedoresGlobais_baterias_v4');
    return saved ? JSON.parse(saved) : [];
  });

  const [colunasCodVisiveis, setColunasCodVisiveis] = useState(() => {
    const saved = localStorage.getItem('colunas_cod_visiveis_baterias_v4');
    return saved ? JSON.parse(saved) : {};
  });

  const [coresFornecedores, setCoresFornecedores] = useState(() => {
    const saved = localStorage.getItem('cores_fornecedores_baterias_v4');
  ...
  const [lojasGlobais, setLojasGlobais] = useState(() => {
    const saved = localStorage.getItem('lojasGlobais_baterias');
  ...
  useEffect(() => { localStorage.setItem('visibilidade_colunas_baterias_v4', JSON.stringify(visibilidadeColunas)); }, [visibilidadeColunas]);
  useEffect(() => { localStorage.setItem('fornecedoresGlobais_baterias_v4', JSON.stringify(fornecedoresGlobais)); }, [fornecedoresGlobais]);
  useEffect(() => { localStorage.setItem('colunas_cod_visiveis_baterias_v4', JSON.stringify(colunasCodVisiveis)); }, [colunasCodVisiveis]);
  useEffect(() => { localStorage.setItem('cores_fornecedores_baterias_v4', JSON.stringify(coresFornecedores)); }, [coresFornecedores]);
  useEffect(() => { localStorage.setItem('lojasGlobais_baterias', JSON.stringify(lojasGlobais)); }, [lojasGlobais]);
  ...
  const carregarMesa = async () => {
    ...
    try {
      const t = Date.now();
      let url = `/mesacompras/BATERIAS?page=${paginaAtual}&size=${ITENS_POR_PAGINA}&_t=${t}`;
  ...
  const carregarMarcasGlobais = async () => {
    try {
      const res = await api.get(`/mesacompras/marcas/BATERIAS?_t=${Date.now()}`);
  ...
  const handleSalvarNovoProduto = async () => {
    ...
    const payload = {
      ...
      categoria: 'BATERIAS', categoriaAba: 'BATERIAS',
  ...
  const getPrecoCarrinho = (p, fornId) => {
    ...
    if (fornUpper.includes('DIAMONDS') || fornUpper === 'DIAMONS') return getCustoAtualSD(p, 'DIAMONDS'); 
  ...
  const getValorColuna = (p, colId) => {
    const diamEstq = p.estoqueFornecedores?.['DIAMONDS']?.qtd || p.estoqueFornecedores?.['DIAMONS']?.qtd || 0;
    ...
    const cAntDiam = p.estoqueFornecedores?.['DIAMONDS']?.vUnit || p.estoqueFornecedores?.['DIAMONS']?.vUnit || 0;
    const cAtualDiam = getCustoAtualSD(p, 'DIAMONDS') || getCustoAtualSD(p, 'DIAMONS');
  ...
  return (
    <div className="h-full flex flex-col bg-[#0B1120] p-6 overflow-hidden text-slate-200 relative">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6 shrink-0">
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-wider flex items-center gap-3">
            <Battery className="text-emerald-400" size={32} /> MESA DE COMPRAS: BATERIAS
          </h1>
          ...
          <table className="w-full border-collapse text-left whitespace-nowrap min-w-max">
            <thead className="sticky top-0 z-30 bg-[#111A2C] text-[10px] font-black uppercase tracking-wider">
              ...
              <tr className="border-b-2 border-slate-700">
                {/* 🔥 CÓDIGOS DINÂMICOS (SÓ OS QUE EXISTEM NO EXCEL!) */}
      ...
      {/* 🔥 MODAL COLUNAS CÓD. COM ESCOLHA DE CORES E ORDENAÇÃO! */}
      {modalAtivo === 'COLUNAS' && (
      ...
      {/* OUTROS MODAIS (DELETE, FORNECEDORES, NOVO PRODUTO, CARRINHO) ABAIXO... */}
