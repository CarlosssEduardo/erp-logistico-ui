...
import { 
  Cpu, Columns, Store, ShoppingCart, 
...
export default function MesaComponentes() {
  ...
  const [visibilidadeColunas, setVisibilidadeColunas] = useState(() => {
    const saved = localStorage.getItem('visibilidade_colunas_componentes_v4');
    return saved ? JSON.parse(saved) : {
      modelo: true, v_mes: true, v_sem: true, media_sem: true,
  ...
  const toggleVisibilidade = (chave) => setVisibilidadeColunas(prev => ({ ...prev, [chave]: prev[chave] === undefined ? false : !prev[chave] }));
  const isVisible = (chave) => visibilidadeColunas[chave] !== false; 

  const [fornecedoresGlobais, setFornecedoresGlobais] = useState(() => {
    const saved = localStorage.getItem('fornecedoresGlobais_componentes_v4');
    return saved ? JSON.parse(saved) : [];
  });

  const [colunasCodVisiveis, setColunasCodVisiveis] = useState(() => {
    const saved = localStorage.getItem('colunas_cod_visiveis_componentes_v4');
    return saved ? JSON.parse(saved) : {};
  });

  const [coresFornecedores, setCoresFornecedores] = useState(() => {
    const saved = localStorage.getItem('cores_fornecedores_componentes_v4');
  ...
  const [lojasGlobais, setLojasGlobais] = useState(() => {
    const saved = localStorage.getItem('lojasGlobais_componentes');
  ...
  useEffect(() => { localStorage.setItem('visibilidade_colunas_componentes_v4', JSON.stringify(visibilidadeColunas)); }, [visibilidadeColunas]);
  useEffect(() => { localStorage.setItem('fornecedoresGlobais_componentes_v4', JSON.stringify(fornecedoresGlobais)); }, [fornecedoresGlobais]);
  useEffect(() => { localStorage.setItem('colunas_cod_visiveis_componentes_v4', JSON.stringify(colunasCodVisiveis)); }, [colunasCodVisiveis]);
  useEffect(() => { localStorage.setItem('cores_fornecedores_componentes_v4', JSON.stringify(coresFornecedores)); }, [coresFornecedores]);
  useEffect(() => { localStorage.setItem('lojasGlobais_componentes', JSON.stringify(lojasGlobais)); }, [lojasGlobais]);
  ...
  const [novoProduto, setNovoProduto] = useState({ modelo: '', descricaoLimpa: '', codigos: {}, custos: {}, quantidades: {} });
  ...
  const carregarMesa = async () => {
    ...
    try {
      const t = Date.now();
      let url = `/mesacompras/COMPONENTES?page=${paginaAtual}&size=${ITENS_POR_PAGINA}&_t=${t}`;
      ...
      const itensCompletos = await Promise.all(baseItems.map(async (item) => {
        ...
        const normEstq = {};
        Object.entries(item.estoqueFornecedores || {}).forEach(([k, v]) => { normEstq[normalizarForn(k)] = v; });
  ...
  const carregarMarcasGlobais = async () => {
    try {
      const res = await api.get(`/mesacompras/marcas/COMPONENTES?_t=${Date.now()}`);
  ...
  const handleSalvarNovoProduto = async () => {
    if (!novoProduto.descricaoLimpa || !novoProduto.modelo) return mostrarToast("Preencha o Modelo e a Descrição Limpa!", "erro");
    ...
    const payload = {
      ...
      modelo: novoProduto.modelo.trim().toUpperCase(),
      categoria: 'COMPONENTES', categoriaAba: 'COMPONENTES',
    ...
    try {
      ...
      mostrarToast("Novo componente injetado na mesa com sucesso!");
      setModalAtivo(null);
      setNovoProduto({ modelo: '', descricaoLimpa: '', codigos: {}, custos: {}, quantidades: {} });
      carregarMesa();
  ...
  const handleAddCarrinho = (produto) => {
    ...
    const nomeComModelo = `${produto.modelo || ''} ${produto.descricaoLimpa}`.trim();
    ...
    if (indexExistente >= 0) {
      ...
      novoCarrinho.push({
        id: Date.now(), produtoId: produto.id, descricaoLimpa: nomeComModelo,
  ...
  const handleFinalizarPedido = async () => {
    ...
    try {
      ...
      carregarMesa();
    } catch (e) { mostrarToast("Erro ao registrar o pedido no banco de dados.", "erro"); }
  };
          ...
          <th className={`${baseClasses} relative group`}>
              ...
              {menuFiltroAberto === id && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-[#0B1120] border border-cyan-900 rounded-xl shadow-2xl py-2 z-100 min-w-42.5 text-left font-bold" onClick={e => e.stopPropagation()}>
                      <div className="px-4 py-2.5 hover:bg-cyan-600 hover:text-white cursor-pointer text-xs text-slate-300 flex items-center gap-2" onClick={() => { setOrdenacao({ coluna: id, direcao: 'asc' }); setMenuFiltroAberto(null); }}><ArrowUp size={14}/> {id === 'modelo' ? 'A - Z' : 'Menor para Maior'}</div>
                      <div className="px-4 py-2.5 hover:bg-cyan-600 hover:text-white cursor-pointer text-xs text-slate-300 flex items-center gap-2 border-b border-slate-800" onClick={() => { setOrdenacao({ coluna: id, direcao: 'desc' }); setMenuFiltroAberto(null); }}><ArrowDown size={14}/> {id === 'modelo' ? 'Z - A' : 'Maior para Menor'}</div>
                      {id !== 'modelo' && (
                        <>
  ...
  return (
    <div className="h-full flex flex-col bg-[#0B1120] p-6 overflow-hidden text-slate-200 relative">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6 shrink-0">
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-wider flex items-center gap-3">
            <Cpu className="text-emerald-400" size={32} /> MESA DE COMPRAS: COMPONENTES
          </h1>
          <p className="text-sm text-slate-400 font-bold mt-1 ml-11">Arquitetura dinâmica com filtro de modelos e cálculo por fornecedor.</p>
        </div>
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
          ...
          <table className="w-full border-collapse text-left whitespace-nowrap min-w-max">
            <thead className="sticky top-0 z-30 bg-[#111A2C] text-[10px] font-black uppercase tracking-wider">
              <tr>
                <th colSpan={qtdColunasVisiveis + (isVisible('modelo') ? 2 : 1)} className="sticky left-0 z-40 bg-[#090E17] border-b border-r-[3px] border-[#334155] text-center text-blue-400 p-2 shadow-[4px_0_10px_rgba(0,0,0,0.6)]">Identificação Cruzada Automática</th>
              ...
              <tr className="border-b-2 border-slate-700">
                ...
                {/* 🔥 COLUNA MODELO INSERIDA AQUI NO MEIO DA IDENTIFICAÇÃO */}
                {isVisible('modelo') && (
                  <th style={{ left: `${qtdColunasVisiveis * 100}px` }} className="sticky z-40 bg-[#090E17] border-r border-[#1E293B] p-2 min-w-35 w-35 group">
                     <div className="flex items-center justify-center gap-1.5 cursor-pointer hover:text-white transition-colors select-none mt-3" onClick={(e) => { e.stopPropagation(); setMenuFiltroAberto(menuFiltroAberto === 'modelo' ? null : 'modelo'); }}>
                          <span className="text-cyan-400">Modelo</span>
                          <Filter size={12} className={(filtrosNumericos['modelo'] || ordenacao.coluna === 'modelo') ? 'text-cyan-400' : 'text-slate-600 group-hover:text-slate-400'} />
                     </div>
                     {menuFiltroAberto === 'modelo' && (
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-[#0B1120] border border-cyan-900 rounded-xl shadow-2xl py-2 z-100 min-w-42.5 text-left font-bold" onClick={e => e.stopPropagation()}>
                            <div className="px-4 py-2.5 hover:bg-cyan-600 hover:text-white cursor-pointer text-xs text-slate-300 flex items-center gap-2" onClick={() => { setOrdenacao({ coluna: 'modelo', direcao: 'asc' }); setMenuFiltroAberto(null); }}><ArrowUp size={14}/> A - Z</div>
                            <div className="px-4 py-2.5 hover:bg-cyan-600 hover:text-white cursor-pointer text-xs text-slate-300 flex items-center gap-2 border-b border-slate-800" onClick={() => { setOrdenacao({ coluna: 'modelo', direcao: 'desc' }); setMenuFiltroAberto(null); }}><ArrowDown size={14}/> Z - A</div>
                        </div>
                     )}
                  </th>
                )}
                
                <th style={{ left: `${(qtdColunasVisiveis * 100) + (isVisible('modelo') ? 140 : 0)}px` }} className="sticky z-40 bg-[#090E17] border-r-[3px] border-[#334155] p-2 min-w-62.5 shadow-[4px_0_10px_rgba(0,0,0,0.6)]">
                  <div className="flex flex-col gap-2">
            ...
            <tbody className="divide-y divide-[#1E293B] text-[11px] font-bold text-slate-300">
              {loading ? (
                <tr><td colSpan="40" className="p-12 text-center text-cyan-400"><Loader2 className="animate-spin inline mr-2" size={24}/> Carregando matemática...</td></tr>
              ) : produtosProcessados.length === 0 ? (
                 <tr><td colSpan="40" className="p-8 text-center text-slate-500">Nenhum produto atende aos filtros atuais.</td></tr>
              ) : (
                produtosProcessados.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className={rowClasses}>
                      ...
                      {isVisible('modelo') && (
                        <td style={{ left: `${qtdColunasVisiveis * 100}px` }} className={`py-4 sticky z-20 ${inCart ? 'bg-[#091512]' : 'bg-[#0C121E]'} group-hover:bg-[#131D2F] border-r border-[#1E293B] px-3 text-center text-cyan-500 font-black`}>
                          {p.modelo || '-'}
                        </td>
                      )}

                      <td style={{ left: `${(qtdColunasVisiveis * 100) + (isVisible('modelo') ? 140 : 0)}px` }} className={`py-4 sticky z-20 ${inCart ? 'bg-[#091512] text-emerald-400' : 'bg-[#0C121E] text-white'} group-hover:bg-[#131D2F] border-r-[3px] border-[#334155] px-3 font-black shadow-[4px_0_10px_rgba(0,0,0,0.6)] transition-colors`}>{p.descricaoLimpa}</td>
      ...
      {/* 🔥 MODAL COLUNAS CÓD (CORES E ORDENAÇÃO) */}
      {modalAtivo === 'COLUNAS' && (
      ...
      {/* 🔥 SUPER MODAL DE VISIBILIDADE DE COLUNAS FIXAS */}
      {modalAtivo === 'VISIBILIDADE' && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-[#0E1525] border border-fuchsia-900/50 rounded-2xl shadow-[0_0_40px_rgba(192,38,211,0.15)] w-full max-w-4xl p-6 relative flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 py-2 grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex flex-col gap-2">
                {/* INCLUSO: Visibilidade do Modelo se precisar */}
                <h3 className="text-[10px] font-black text-cyan-400 uppercase tracking-widest mt-4 mb-1 border-b border-slate-800 pb-1">Identificação</h3>
                <button onClick={() => toggleVisibilidade('modelo')} className={`flex justify-between items-center w-full px-4 py-2 rounded-lg border font-black text-[11px] uppercase transition-all ${isVisible('modelo') ? 'bg-cyan-900/20 border-cyan-500/50 text-cyan-400' : 'bg-[#0B1120] border-slate-700 text-slate-500 hover:border-slate-500'}`}>
                  MODELO {isVisible('modelo') ? <Eye size={14}/> : <EyeOff size={14}/>}
                </button>

                <h3 className="text-[10px] font-black text-fuchsia-500 uppercase tracking-widest mt-4 mb-1 border-b border-slate-800 pb-1">Giro de Vendas</h3>
              ...
              <div className="flex flex-col gap-2">
                ...
                <h3 className="text-[10px] font-black text-cyan-400 uppercase tracking-widest mt-4 mb-1 border-b border-slate-800 pb-1">Edição Rápida</h3>
      ...
      {/* MODAL NOVO PRODUTO E CARRINHO ABAIXO... */}
          ...
          <div className="bg-[#0E1525] border border-[#1E293B] rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col overflow-hidden max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto bg-[#0B1120] flex-1">
              <div className="flex gap-4 mb-6">
                <div className="w-1/3">
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Modelo da Peça</label>
                  <input type="text" value={novoProduto.modelo} onChange={e => setNovoProduto({...novoProduto, modelo: e.target.value.toUpperCase()})} placeholder="Ex: CAMERA TRAS." className="w-full bg-[#0E1525] border border-slate-700 text-cyan-400 rounded-xl p-3 text-sm font-black outline-none focus:border-cyan-500" />
                </div>
                <div className="flex-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Nome do Produto (Descrição Limpa)</label>
                  <input type="text" value={novoProduto.descricaoLimpa} onChange={e => setNovoProduto({...novoProduto, descricaoLimpa: e.target.value.toUpperCase()})} placeholder="Ex: IPHONE 11 PRO MAX" className="w-full bg-[#0E1525] border border-slate-700 text-white rounded-xl p-3 text-sm font-black outline-none focus:border-cyan-500" />
                </div>
              </div>
          ...
          <div className="bg-[#0E1525] border border-[#1E293B] rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden max-h-[90vh]">
            <div className="p-6 border-b border-slate-800 flex justify-between items-start">
              <div className="flex gap-4 items-center w-full">
                ...
                <div className="flex-1">
                  <h2 className="text-xl font-black text-white uppercase flex items-center gap-2 mb-3">Carrinho Rápido - {fornecedorAtivo}</h2>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative group shrink-0 w-64 z-50">
                      <div className="flex items-center gap-2 bg-[#0B1120] border border-slate-700 rounded-lg px-3 py-2 cursor-pointer group-hover:border-cyan-500 transition-colors" onClick={(e) => { e.stopPropagation(); setMenuLojaAberto(!menuLojaAberto); }}>
                        ...
                        <span className="text-cyan-400 font-black text-xs uppercase flex-1 truncate">{lojaSelecionada || 'S / LOJA (CADASTRAR)'}</span>
            ...
            <div className="flex-1 overflow-auto bg-[#0B1120] p-6 z-0">
              {carrinho.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 opacity-50">
                  ...
                  <p className="text-slate-400 font-bold">O carrinho está vazio.</p>
                </div>
              ) : (
